 ...
 *
 * Original written by Philipp von Weitershausen <philipp@weitershausen.de>
 * Released under the MIT license.
 * http://www.opensource.org/licenses/mit-license.php
 *
 * Tweaked by Max Brosnahan.
 *
...
function MockHttpServer (handler) {
    this.passThroughRules = [];
    if (handler) {
        this.handle = handler;
    }
    /**
     * Object containing all the requests that have been made
     */
    this.requests = [];
};
MockHttpServer.prototype = {
    ...
    preHandle: function(request) {
        this.requests.push(request);

        for (var i = 0; i < this.passThroughRules.length; i++) {
            var passThroughRule = this.passThroughRules[i];

            if(passThroughRule.test(request.url)) {
                return this.passThrough(request, window.OriginalHttpRequest);
            }
        }
        return this.handle(request);
    },

    // Add a url rule (regexp or string) that will determine if it should be passed through
    addPassThroughRule: function (rule) {
        if(!rule instanceof RegExp) {
            rule = new RegExp(rule);
        }
        this.passThroughRules.push(rule)
    },
    // Pass the request through to an actual XMLHttpRequest object.
    passThrough: function passThrough(fakeRequest) {
        var requestSpy = this.requestSpy;

        var actualRequest = new window.OriginalHttpRequest();
        actualRequest.onreadystatechange = function() {
            // Copy the request state across to the fake one which is the one used by angular
            fakeRequest.readyState = actualRequest.readyState;
            if(actualRequest.readyState === 4){
                fakeRequest.status = actualRequest.status;
                fakeRequest.responseText = actualRequest.responseText;
                fakeRequest.responseXML = actualRequest.responseXML;
            }

            // Call the state change handler in angular
            fakeRequest.onreadystatechange();

            // If there is a function spying on the request call it
            requestSpy && requestSpy(fakeRequest);
        }
        // Open a request with the data from the fakeRequest
        actualRequest.open(fakeRequest.method, fakeRequest.url, fakeRequest.async);

        var requestHeaders = fakeRequest.requestHeaders
        // Copy all the request headers into the actual request
        for(var header in requestHeaders) {
          if(requestHeaders.hasOwnProperty(header)) {
            actualRequest.setRequestHeader(header, requestHeaders[header]);
          }
        }

        actualRequest.send(fakeRequest.requestText);
    }
};
